...
Runs a scripted sequence on a loop, mirroring LED and LED II's own five-beat structure -- confirmed with Bruce (2026-08-24), since (like LED II) Title had no source spec of its own pinning down a script: power-up (random bit flicker across both strips, then a column sweep that speeds up), a scrolling main-content phase (both strips' value-per-column mapping scroll over time, in opposite directions at different speeds, starting each time from offset 0 -- the exact identity ramp `TITLE.png` itself shows), a snake-chase minigame on each strip's bit grid independently, a firework burst on both strips together, then a held "1991" credit. Unlike LED/LED II, Title has no font to hold that credit with (a `BitColumnDisplay` column is a byte value, not a glyph cell), so `WordsPhase` lights the matching `(col, row)` cells of `led_grid.DOT_FONT`'s digit glyphs directly via `render_raw`, the same "addressing individual cells" role that phase already plays for the snake and fireworks. See `retrodemos/demos/title_phases.py` for each phase's choreography.

The snake and fireworks phases address individual `(column, bit)` cells directly via `BitColumnDisplay.render_raw` (alongside the byte-value-driven `render_values` the scroll phase uses) and `framework/graph_walk.py`'s `ChaseSnake`/`ChasePair`/`ChaseMatch`/`Burst`/`Rocket` over `led_grid.dot_grid_adjacency(width, ROWS)` -- the same primitives LED II's phases use, over a bit grid instead of a dot grid. `dot_grid_adjacency` is purely `(col, row)` topology with no dot-specific assumptions, so it's reused as-is rather than duplicated for a "bit grid" variant.

Playtesting (2026-08-26) applied the same treatment LED II's own phases got that day: the snake scene on each strip is now a best-of-3 match (`graph_walk.ChaseMatch`) rather than a single round, with score shown as dots stacked on each snake's own starting side of its strip; `CHASE_CHANCE` was lowered (0.65 -> 0.45) for more randomized paths; the fireworks burst on each strip now gets a `graph_walk.Rocket` launch trail climbing from the bottom row to its target before igniting; and the scroll phase's two intervals were sped up (0.03/0.045 -> 0.02/0.03).
