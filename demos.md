...
- All demos are automated attract-mode, except **Bruce's Windows** and **CD Player**, which are functionally interactive (CD Player's one window is draggable and its transport buttons/body are clickable; it briefly had a separate equalizer companion window too, removed 2026-08-26 playtesting once the EQ display moved onto the main pane). Neither's chrome is shared with other demos; see `PLAN.md`.
...
- Desktop shell (`retrodemos/demos/desktop.py`, spec logged 2026-08-24, built 2026-08-25 -- see `PLAN.md`'s "Future: the unified desktop"): `python -m retrodemos` with no name now opens a 1024x576 desktop with one icon per built demo, laid out in a single left-aligned column; click an icon to open it as its own draggable/closable window (`framework/window_chrome.py`), the same as a real desktop would host several little utility programs at once. `python -m retrodemos <name>` still runs any one demo standalone. A macOS-style top menu bar (added 2026-08-25) sits above every window: white, exactly a line of `pixel_font` text tall, bold (no shadow -- an earlier drop-shadow version was replaced once seen live), a ⌘ icon with a functional dropdown (About / Close All Windows / Quit) at the left, and the focused window's name after it, or "HELP" (click it for a condensed README panel) when nothing is focused. CD Player and Tank Status Window both open chromeless (`desktop.py`'s `_CHROMELESS`) since each already draws its own complete window frame -- wrapping either in the desktop's generic chrome nested a window inside a window (caught in playtesting). An open demo's icon dims instead of disappearing (2026-08-25: disappearing read as "weird"); Bruce's Windows' own icon is hidden from the grid entirely rather than just dimmed (2026-08-26 playtesting), a demo of windowing chrome feeling redundant on a desktop that's itself a real windowing system.
...
Per-demo open questions now live in each spec file. All 8 planned demos are built and have been through at least one playtesting pass (2026-08-26); `docs/bruces-windows.md`, `docs/cd-player.md`, and `docs/cinqtris.md` each still note a small unresolved item of their own -- see those files directly.
