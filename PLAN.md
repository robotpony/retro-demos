...
Groundwork done 2026-08-25, ahead of the desktop itself: `WINDOW1.png` review identified two distinct window archetypes (Main Window: outer frame, title bar, corner boxes, status bar; Dialog: lighter border, title bar only, no other chrome -- see `docs/bruces-windows.md`) and four measured, reusable border-style helpers (`bruces_windows.py`'s `_bevel_rect`, `_ring_frame`, `_double_rule_rect`, plus the outer-frame logic). Not extracted into `framework/` yet -- still one demo's own code, written generically enough to lift out once the desktop (or a second window-drawing demo) actually needs them.
...
Build order 5 (Bruce's Windows) is done (2026-08-25), the first interactive demo. `WINDOW1.png` is a single coherent screenshot (unlike Title/Dooley/CD Player's source images), so no calibration-band untangling was needed; every text label (window title, dialog title, its two body lines, the button label, the status bar text) is pixel-verified -- extracted as the literal set of black pixels within its own tight bounding box, not decomposed into a reusable font, since every label here is fixed content. Caught and fixed one extraction bug along the way: an overly-tight search bound truncated the status text's "T" (its crossbar and stem), initially reading "his is a status a bar..." instead of the source's actual (and genuinely typo'd) "This is a status a bar...". Window chrome was corrected on a follow-up review the same day (request: identify the reusable window building blocks, since a first pass at bevel geometry wasn't accurate). `WINDOW1.png` turned out to show two distinct window archetypes, not one window with a popup bolted on: a **Main Window** (outer frame, title bar with a sunken-bevel frame, two raised-bevel corner boxes, a status bar) and a lighter-weight **Dialog** (plain black+white border, same cyan title bar style, no corner boxes/status bar/resize grip) -- see `docs/bruces-windows.md`'s new "Two window archetypes" section for the full component inventory. The chrome itself uses four distinct border styles, not one generic bevel the first pass had collapsed them into (and got backwards -- dark top/left instead of light top/left for "raised"): a simple 1px bevel (`_bevel_rect`, corner boxes and both title bars' frames), a triple-ring frame (`_ring_frame`, the "Got it" button -- grey/black/white, outside-in, uniform on all sides, not directional at all), and a double-rule border (`_double_rule_rect`, the status text field and icon-grid box -- separate grey and white lines with background gaps, not a touching bevel). Every style is now measured by sampling its edge profile outside-in on all four sides rather than eyeballed. The status bar's own icon grid (12x4 cells) is pixel-verified, since its exact pattern is content, not decoration. Needed one new piece of shared framework, not just this demo's own code: `runtime.py` now rescales mouse event `pos`/`rel` from window-pixel space to native-canvas space before a demo ever sees them (`_to_native_space`), since no prior demo needed mouse input at all -- this benefits Cinqtris's About button next for free. The demo's own canvas (320x240) is bigger than the window (200x200) so there's visible room to drag it; the surrounding desktop backdrop is an invented flat teal, since `WINDOW1.png` shows only the window itself. The four border-style helpers aren't extracted into `framework/` yet -- written generically enough to lift out once a second window-drawing demo needs them, not done speculatively ahead of that. 140 tests passing (`tests/`).
