...
Not every demo has an icon -- only the six built so far (LED, LED II, Title, CD Player, Bruce's Windows, Cinqtris). Bruce's 21 and Tank Status Window join `desktop.py`'s `_DEMO_ENTRIES` list as each is built.
...
Build order 7: Bruce's 21 (`CARDS.png`, `BACKS.png`). Sprite art plus phase-cycling (deck cycle, then auto-deal), no interaction. Check `docs/bruces-21.md` and do its own pixel-archaeology pass before assuming anything carries over -- every demo so far has held at least one real surprise in its source image. Once built, add it to `desktop.py`'s `_DEMO_ENTRIES` (icon glyph, mocked up and confirmed first) so the desktop shell can open it too.
