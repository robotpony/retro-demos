...
`framework/led_grid.py` is shared by Dooley, LED, LED II, and Title. The original plan (below, kept for history) was one generic cell-grid renderer for all four; in practice each demo's source image turned out to need its own renderer shape closely matched to that image's actual pixel model, not a single grid abstraction bent to fit all three -- `SevenSegmentDisplay`, `DotMatrixDisplay`, and `BitColumnDisplay` are three distinct classes in the same module, not three configurations of one class. Dooley's colour side-column is still open; decide whether it fits one of these or needs its own from its own source image, not by assumption:
...
- **Snake-chase minigame, not just a wandering snake.** Title's `SnakePhase` was rebuilt 2026-08-24 (request: the scene ran too short and didn't cover enough horizontal ground) from a single unbiased `graph_walk.Snake` per strip into a chase minigame: two `graph_walk.ChaseSnake`s per strip, spawned a guaranteed quarter-width apart, hunt each other's head (target-weighted steps, biased toward whichever axis needs closing most) until one catches the other, flashes a few times, and hands off to the next phase. See `retrodemos/demos/title_phases.py`'s `SnakePhase`/`_ChasePair`/`_chase_distance` for the full implementation and rationale. LED's and LED II's own `SnakePhase`s still use the plain wandering `Snake` -- worth porting this same pattern to them next time either is touched, tuning `ChaseSnake`'s `distance` weighting and the spawn-separation rule for each one's own grid shape (LED's segment graph isn't spatial the way a dot grid is, so its "opposite quarters" spawn rule and axis-weighted distance function will need their own equivalent, not a verbatim copy).
...
Title turned out not to fit `DotMatrixDisplay` after all, despite that being the plan going in (see "LED grid module" above for the corrected per-demo table): its source image (`TITLE.png`) has no bezel, no gap between columns, and content directly computable from a byte value rather than drawn from a font, so it got its own renderer (`BitColumnDisplay`) instead of a forced extension. `TITLE.png` also turned out to encode the actual rendering *rule* (column x shows value x's own bits), not just a lit/unlit calibration image like LED's and LED II's source images -- verified pixel-exact against all 256 columns x 8 rows x 2 colour pairs. Dooley's dot-matrix-plus-colour-side-column need still isn't built; given Title's outcome, don't assume it fits `DotMatrixDisplay` either -- check its own source image first.

## Next step

Build order 4: Dooley. Same framework, two content streams (main display + side column) -- do the pixel-archaeology pass on its source image first to determine whether its dot-matrix need fits `DotMatrixDisplay` as-is, needs extending, or (per Title's precedent) needs its own renderer, per `docs/dooley.md`. `Phase`/`PhaseSequence`/`Ticker`/`graph_walk` are all there already if Dooley's spec turns out to want a scripted sequence, which every demo built so far has ended up wanting.
