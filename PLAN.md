...
Not every demo has an icon -- only the seven built so far (LED, LED II, Title, CD Player, Bruce's Windows, Cinqtris, Bruce's 21). Tank Status Window joins `desktop.py`'s `_DEMO_ENTRIES` list once it's built.
...
- Only 7 of the 9 planned demos have desktop icons; Tank Status Window joins as it's built (order 8 in the "Build order" table above).
